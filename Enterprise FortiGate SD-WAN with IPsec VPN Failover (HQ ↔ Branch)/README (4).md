...
```
        ...
        ┌───────────────────────────┬───────────────────────────┐
        │                           │                            │
 ISP-HQ MikroTik              ISP-Branch MikroTik         ISP-Branch2 MikroTik
 WAN 192.168.8.40             WAN 192.168.8.50            WAN 192.168.8.60
 LAN 203.0.113.1/24           LAN 198.51.100.1/24         LAN 198.51.101.1/24
        │                           │                            │
 FW-HQ-01 (WAN2/port3)       FW-BR-01 (WAN2/port3)       FW-BR-02 (WAN2/port3)
 203.0.113.2/24               198.51.100.2/24             198.51.101.2/24
        │                           │                            │
 FW-HQ-01 (WAN1/port1)       FW-BR-01 (WAN1/port1)       FW-BR-02 (WAN1/port1)
 192.168.8.20/24              192.168.8.50/24             192.168.8.60/24
 (direct on shared segment)   (direct on shared segment)  (direct on shared segment)
        │                           │                            │
 FW-HQ-01 LAN (port2)        FW-BR-01 LAN (port2)        FW-BR-02 LAN (port2)
 10.0.0.1/24                  172.16.0.1/24               172.17.0.1/24
```

Two IPsec tunnels connect each branch to HQ — one riding WAN1 (direct on the
...
| Branch1 | 172.16.0.1/24  | 192.168.8.50/24   | 198.51.100.2/24     | 198.51.100.1/24   |
| Branch2 | 172.17.0.1/24  | 192.168.8.60/24   | 198.51.101.2/24     | 198.51.101.1/24   |
...
| Branch1 | ESXi            | ESXi               |
| Branch2 | Hyper-V         | Hyper-V (planned)  |
...
- **Once SD-WAN is fully established, the main outstanding issue is packet
  ...
  across all sites) ultimately break out through the same single real public
  IP on the shared home router, since every simulated ISP MikroTik is itself
  behind that one uplink. With no genuine path diversity, the Performance SLA
